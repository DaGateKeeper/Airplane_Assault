...
public class BaseActor extends Group
{
    ...
    private int reflectX;
    private int reflectY;
    ...
    public void draw(Batch batch, float parentAlpha) 
    {
        ...
        if ( animator != null && isVisible() )
            batch.draw( 
                ...
                getWidth(), getHeight(), 
                getScaleX() * reflectX, getScaleY() * reflectY, 
                getRotation() );
    ...
    public boolean isOnScreen(float worldWidth, float worldHeight)
    {
        // check each edge
        if ( getX() < -getWidth() )
            return false;
        else if ( getX() > worldWidth )
            return false;
        else if ( getY() < -getHeight() )
            return false;
        else if ( getY() > worldHeight )
            return false;
        else
            return true;
